...
async function addLead() {
  try {
    const response = await axios.post('https://trywhiteowl.com/api/v1/leads', {
      name: 'John Doe',
      email: 'john@gmail.com',
      source: 'website',
      note: 'Might be interested in React course'
...
def add_lead():
    url = 'https://trywhiteowl.com/api/v1/leads'
    headers = {
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json'
    }
...
```bash
curl -X POST https://trywhiteowl.com/api/v1/leads \
...
async function updateLead(leadId) {
  try {
    const response = await axios.put(`https://trywhiteowl.com/api/v1/leads/${leadId}`, {
      email: 'john.new@gmail.com',
      note: 'Followed up today - very interested in React course'
    }, {
      headers: {
...
def update_lead(lead_id):
    url = f'https://trywhiteowl.com/api/v1/leads/{lead_id}'
    headers = {
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json'
    }
...
```bash
curl -X PUT https://trywhiteowl.com/api/v1/leads/lead_12345 \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "john.new@gmail.com",
...
async function deleteLead(leadId) {
  try {
    const response = await axios.delete(`https://trywhiteowl.com/api/v1/leads/${leadId}`, {
      headers: {
        'Authorization': 'Bearer YOUR_API_KEY'
      }
    });
...
```bash
curl -X DELETE https://trywhiteowl.com/api/v1/leads/lead_12345 \
  -H "Authorization: Bearer YOUR_API_KEY"
