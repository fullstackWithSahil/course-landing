...
async function sendEmail() {
  try {
    const response = await axios.post('https://trywhiteowl.com/api/v1/email/send', {
      to: 'student@example.com',
      subject: 'Welcome to the course!',
      from: 'teacher@example.com',
      html: '<p>This is an example!</p>'
...
def send_email():
    url = 'https://trywhiteowl.com/api/v1/email/send'
    headers = {
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json'
    }
...
```bash
curl -X POST https://trywhiteowl.com/api/v1/email/send \
...
async function listTemplates() {
  try {
    const response = await axios.get('https://trywhiteowl.com/api/v1/email/template', {
      headers: {
        'Authorization': 'Bearer YOUR_API_KEY'
      }
    });
...
def list_templates():
    url = 'https://trywhiteowl.com/api/v1/email/template'
    headers = {
        'Authorization': 'Bearer YOUR_API_KEY'
    }
...
```bash
curl -X GET https://trywhiteowl.com/api/v1/email/templates \
  -H "Authorization: Bearer YOUR_API_KEY"
...
async function sendTemplateEmail() {
  try {
    const response = await axios.post('https://trywhiteowl.com/api/v1/email/send-template', {
      to: 'student@example.com',
      template_id: 'template_123',
      variables: {
        student_name: 'John Doe',
...
def send_template_email():
    url = 'https://trywhiteowl.com/api/v1/email/send-template'
    headers = {
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json'
    }
...
```bash
curl -X POST https://trywhiteowl.com/api/v1/email/send-template \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "to": "student@example.com",
